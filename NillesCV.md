...
Description:
:   Worked closely with engineers to develop hardware and
software for their mobile platform. Configured a ROS server and added a wifi module to the robot
to stream data through ROS. Analyzed the resulting data using Python, to
compare streaming pose estimates from the robot with a ground truth from a
motion capture system (also ROS-integrated). Analyzed how the robot slipped on
different surfaces to help improve low-level controllers.
